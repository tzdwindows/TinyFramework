...
static const char *mini_js_shim =
    ...
    /* innerText: THE click fix. Previously undefined, so a handler that did
       ...
       the same _getText/_setText (textContent) path so the tree updates. */
    ...
    /* Event / CustomEvent constructors usable with dispatchEvent. */
    ...
    /* location / navigator (static best-effort; enough for feature-detection). */
    ...
    "window.navigator={userAgent:'Mozilla/5.0 (TinyFramework) mini-renderer',platform:'Win32',language:'en-US',languages:['en-US','en'],onLine:true,cookieEnabled:false,vendor:'TinyFramework',appVersion:'1.0',product:'Gecko',hardwareConcurrency:4};\n"
    ...
    /* fetch is synchronous here; a thenable makes `await fetch()` and
       `fetch().then(r=>...)` both work (chains resolve synchronously). */
    "function __thenable(v){return {then:function(cb,onRej){try{return __thenable(cb?cb(v):v);}catch(e){if(onRej){try{return __thenable(onRej(e));}catch(_){}}return __thenable(v);}}};};\n";
...
static JSValue build_js_event(JSContext *ctx, const MiniEvent *ev, MiniBridge *b)
{
    ...
    /* DragEvent.dataTransfer: a minimal DataTransfer backed by the C
       dnd.data slot (text/plain). Attached for every drag-family event so
    ...
    if (ev->type && (!strncmp(ev->type, "drag", 4) || !strcmp(ev->type, "drop")))
    {
        static const char *dt_src =
            ...
            "types:['text/plain'],dropEffect:'move',effectAllowed:'all',files:[]})";
...
static JSValue js_gl_getParameter(JSContext *ctx, JSValueConst tv, int argc, JSValueConst *argv)
{
    ...
    {
        typedef const GLubyte *(*PFN)(GLenum);
        PFN f = (PFN)glfwGetProcAddress("glGetString");
        const GLubyte *s = f ? f((GLenum)pname) : NULL;
        return JS_NewString(ctx, s ? (const char *)s : "");
    }
    GLint iv = 0;
    typedef void (*PFN)(GLenum, GLint *);
    PFN f = (PFN)glfwGetProcAddress("glGetIntegerv");
    if (f)
        f((GLenum)pname, &iv);
...
static JSValue js_gl_getExtension(JSContext *ctx, JSValueConst tv, int argc, JSValueConst *argv)
{
    ...
    if (name)
    {
        static const char *have[] = {
            ...
            "WEBGL_lose_context", "ANGLE_instanced_arrays", NULL};
                              ...
                              int argc, JSValueConst *argv)
{
    ...
    PFN f = (PFN)glfwGetProcAddress("glGetError");
    return JS_NewInt32(ctx, f ? (int32_t)f() : 0);
}
static JSValue js_gl_getShaderPrecisionFormat(JSContext *ctx, JSValueConst tv, int argc, JSValueConst *argv)
{
    ...
    JS_ToInt32(ctx, &shaderType, argv[0]);
    JS_ToInt32(ctx, &precType, argv[1]);
    GLint rangeMin = 1, rangeMax = 1, precision = 0;
    /* Desktop GL has no lowp/mediump/highp tiers (everything is highp), so
       advertise real values when the driver exposes glGetShaderPrecisionFormat
       and otherwise report highp as supported. Previously this returned
       precision=0 for every type, so Three.js's getMaxPrecision fell all the
       way down to lowp; MeshStandardMaterial's lighting then underflowed to 0
       and the cube rendered solid black instead of its indigo material. */
    typedef void (*PFN)(unsigned int, unsigned int, int *, int *, int *);
    PFN f = (PFN)glfwGetProcAddress("glGetShaderPrecisionFormat");
    if (f)
        f((unsigned int)shaderType, (unsigned int)precType, &rangeMin, &rangeMax, &precision);
    if (precision == 0)
    {
        switch (precType)
        {
            case 0x8DF2 /*HIGH_FLOAT*/: rangeMax = 62; precision = 23; break;
            case 0x8DF1 /*MEDIUM_FLOAT*/: rangeMax = 15; precision = 10; break;
            case 0x8DF0 /*LOW_FLOAT*/: rangeMax = 1; precision = 8; break;
            case 0x8DF6 /*HIGH_INT*/: case 0x8DF5 /*MEDIUM_INT*/: case 0x8DF4 /*LOW_INT*/:
                rangeMax = 1; precision = 16; break;
            default: break;
        }
    }
    JSValue o = JS_NewObject(ctx);
...
/* Response.text()/json() — read the cached _body string. */
static JSValue js_resp_text(JSContext *ctx, JSValueConst tv, int argc, JSValueConst *argv)
{
    (void)argc;
    (void)argv;
    return JS_GetPropertyStr(ctx, tv, "_body"); /* returns a new ref */
}
static JSValue js_resp_json(JSContext *ctx, JSValueConst tv, int argc, JSValueConst *argv)
{
    ...
    JSValue r = s ? JS_ParseJSON(ctx, s, strlen(s), "<response>") : JS_NULL;
    if (s)
        JS_FreeCString(ctx, s);
    JS_FreeValue(ctx, b);
    return r;
...
{
    (void)tv;
    const char *url = (argc >= 1) ? JS_ToCString(ctx, argv[0]) : NULL;
    if (!url)
    ...
    MiniNetRecord rec;
    /* page origin (for SOP/CORS + cookie scoping) = the live window.location */
    const char *page_origin = NULL;
    {
        JSValue g = JS_GetGlobalObject(ctx);
        JSValue loc = JS_GetPropertyStr(ctx, g, "location");
        JSValue hrefv = JS_GetPropertyStr(ctx, loc, "href");
        if (JS_IsString(hrefv))
            page_origin = JS_ToCString(ctx, hrefv);
        JS_FreeValue(ctx, hrefv);
        JS_FreeValue(ctx, loc);
        JS_FreeValue(ctx, g);
    }
    mini_net_fetch(method, url, hdrs, body, body_len, page_origin, &rec);
    if (page_origin)
        JS_FreeCString(ctx, page_origin);

    JSValue resp = JS_NewObject(ctx);
    JS_SetPropertyStr(ctx, resp, "url", JS_NewString(ctx, url));
    JS_SetPropertyStr(ctx, resp, "status", JS_NewInt32(ctx, rec.status));
    JS_SetPropertyStr(ctx, resp, "ok", JS_NewBool(ctx, rec.status >= 200 && rec.status < 300));
    JS_SetPropertyStr(ctx, resp, "statusText", JS_NewString(ctx, rec.status >= 200 && rec.status < 300 ? "OK" : "Error"));
    JSValue body_str = JS_NewStringLen(ctx, rec.resp_body ? rec.resp_body : "",
                                       rec.resp_body_len);
    ...
    /* headers object: parse the raw response head line by line */
    JSValue hobj = JS_NewObject(ctx);
    if (rec.resp_headers)
    {
        char *dup = mc_strdup(rec.resp_headers);
        if (dup)
        {
            ...
            while (line)
            {
                ...
                if (c)
                {
                    *c++ = '\0';
                    while (*c == ' ' || *c == '\t')
                        c++;
                    JS_SetPropertyStr(ctx, hobj, line, JS_NewString(ctx, c));
                }
                line = strtok(NULL, "\r\n");
            }
            free(dup);
    ...
    /* record into the Network table (steals rec's buffers) */
...
MiniBridge *mini_bridge_create(MiniRenderer *r, MiniDocument *doc)
{
    ...
    JS_SetMemoryLimit(b->rt, 24 * 1024 * 1024); /* 24 MB heap cap */
    JS_SetMaxStackSize(b->rt, 256 * 1024);
    ...
    /* expose document.body / document.documentElement as wrapped elements */
...
static void apply_styles(struct MiniNode *n, MiniBridge *b)
{
    if (!n)
        return;
    if (n->type == MN_ELEMENT_NODE && n->tag && !strcmp(n->tag, "style") && n->text)
        mini_css_apply(b->doc, n->text);
    for (struct MiniNode *c = n->first_child; c; c = c->next_sibling)
        apply_styles(c, b);
}
